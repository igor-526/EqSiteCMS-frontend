...
export const PriceGroupModal: React.FC<PriceGroupModalProps> = ({
  ...
  const isDeleteButtonAvailable = hasPermission(
    PRICE_PAGE_SCOPES_ACTIONS.PRICE_GROUP_DELETE,
  );
  const isUpdateButtonAvailable = hasPermission(
    PRICE_PAGE_SCOPES_ACTIONS.PRICE_GROUP_UPDATE,
  );
  const isAddButtonAvailable = hasPermission(
    PRICE_PAGE_SCOPES_ACTIONS.PRICE_GROUP_CREATE,
  );
  const isNameFieldDisabled = !hasPermission(
    PRICE_PAGE_SCOPES_ACTIONS.PRICE_GROUP_UPDATE_NAME,
  );
  const isDescriptionFieldDisabled = !hasPermission(
    PRICE_PAGE_SCOPES_ACTIONS.PRICE_GROUP_UPDATE_DESCRIPTION,
  );
    ...
    if (isUpdateButtonAvailable) {
      footer.push(
        <Button
          ...
          type="primary"
          onClick={() =>
            onUpdate(selectedPriceGroup.id, {
              name: name,
  ...
  return (
    <Modal
      open={open}
      title={
        selectedPriceGroup ? "Изменить группу услуг" : "Добавить группу услуг"
      }
      onCancel={onClose}
      footer={footer}
    >
      ...
      <div className="mb-6 flex flex-col gap-2">
        <label htmlFor="createPriceGroupDescriptionInput">
