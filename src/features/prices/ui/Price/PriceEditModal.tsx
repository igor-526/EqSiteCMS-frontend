...
import { TableCell, TableCellFormatter, TableColumn, TableRow, TableType } from "@/types/api/table";
...
const ColumnEditPanel: React.FC<ColumnEditPanelProps> = ({ panel, onSave, onCancel, onChange }) => {
    const { draft, mode } = panel;
    const isEditing = mode === "edit";
    const canSave = !!(draft.key && draft.key.trim() && draft.title && draft.title.trim());
...
const TableTabPane: React.FC<TableTabPaneProps> = ({
    ...
    const handleSaveColumn = (finalCol: TableColumn) => {
        if (!colPanel) return;
        const updated = { ...table, columns: [...table.columns], rows: table.rows.map((r) => ({ ...r, cells: { ...r.cells } })) };
        ...
        if (colPanel.columnIndex !== null) {
            ...
            updated.rows.forEach((row) => {
                if (!row.cells[finalCol.key]) {
                    row.cells[finalCol.key] = { value: "", annotation: "", cell_formatter: [] };
                }
            });
        }
    ...
    const handleDeleteRow = (rowIdx: number) => {
        onTableChange({ ...table, rows: table.rows.filter((_, i) => i !== rowIdx) });
    };

    const handleCellChange = (rowIdx: number, colKey: string, field: "value" | "annotation", val: string) => {
        const rows = table.rows.map((r, i) => {
            if (i !== rowIdx) return r;
            return { ...r, cells: { ...r.cells, [colKey]: { ...r.cells[colKey], [field]: val } } };
        });
        onTableChange({ ...table, rows });
    };

    const handleCellFormatterToggle = (rowIdx: number, colKey: string, fmt: TableCellFormatter) => {
        const rows = table.rows.map((r, i) => {
            if (i !== rowIdx) return r;
            ...
            return { ...r, cells: { ...r.cells, [colKey]: { ...cell, cell_formatter: fmts } } };
        });
        onTableChange({ ...table, rows });
    };
    ...
    const dataCols = table.columns.map((col, colIdx) => ({
        title: (
            <div style={{ minWidth: 120 }}>
                ...
                {col.annotation && (
                    <div style={{ fontSize: 11, color: "#999", fontWeight: 400 }}>{col.annotation}</div>
                )}
            </div>
        ),
        dataIndex: col.key,
        key: col.key,
        width: 200,
        render: (_: unknown, record: TableRow & { key: number }, rowIdx: number) => {
            const cell = record.cells[col.key] || { value: "", annotation: "", cell_formatter: [] as TableCellFormatter[] };
            const fmts = cell.cell_formatter || [];
            return (
                <div className="flex flex-col gap-1 p-1 border rounded" style={{ minWidth: 150 }}>
                    <Input
                        value={cell.value}
                        onChange={(e) => handleCellChange(rowIdx, col.key, "value", e.target.value)}
                        placeholder="Значение"
                        size="small"
                        style={{
                            fontWeight: fmts.includes("text_bold") ? 700 : 400,
                            fontStyle: fmts.includes("text_italic") ? "italic" : "normal",
                            textDecoration: fmts.includes("text_underline") ? "underline" : "none",
                        }}
                    />
                    <Input
                        value={cell.annotation}
                        onChange={(e) => handleCellChange(rowIdx, col.key, "annotation", e.target.value)}
                    ...
                    <div className="flex gap-1">
                        <FmtBtn
                            ...
                            onToggle={() => handleCellFormatterToggle(rowIdx, col.key, "text_bold")}
                            title="Жирный"
                        />
                        <FmtBtn
                            label="К"
                            active={fmts.includes("text_italic")}
                            onToggle={() => handleCellFormatterToggle(rowIdx, col.key, "text_italic")}
                            title="Курсив"
                        />
                        <FmtBtn
                            label="П"
                            active={fmts.includes("text_underline")}
                            onToggle={() => handleCellFormatterToggle(rowIdx, col.key, "text_underline")}
                            title="Подчёркнутый"
                        />
                    </div>
                </div>
    ...
    const tableColumns = table.columns.length > 0 ? [numCol, ...dataCols, actionsCol] : [];
    ...
    return (
        <div>
            {/* Sub-header */}
            <div className="flex items-center gap-2 flex-wrap mb-3" style={{ padding: "4px 0" }}>
                <h4 style={{ margin: 0, fontWeight: 600, fontSize: 14 }}>
                    Таблица {tableIndex + 1} · {table.columns.length} колонок · {table.rows.length} строк
                </h4>
                <div className="flex items-center gap-2 ml-auto flex-wrap">
                    <Button size="small" icon={<CopyOutlined />} onClick={onDuplicateTable}>
                        Дублировать
                    </Button>
            ...
            {table.columns.length === 0 ? (
                <div
                    ...
                    style={{ border: "1px dashed #d9d9d9", borderRadius: 8, padding: "40px 0", minHeight: 160 }}
                >
                    <TableOutlined style={{ fontSize: 32, marginBottom: 12, color: "#bbb" }} />
                    <div style={{ fontSize: 14, marginBottom: 12 }}>Нет колонок в таблице</div>
                    <Button type="primary" size="small" icon={<PlusOutlined />} onClick={handleAddColumn}>
                ...
                <div>
                    ...
                    <div
                        className="flex flex-col items-center justify-center text-gray-400"
                        style={{ border: "1px dashed #d9d9d9", borderRadius: 8, padding: "20px 0", marginTop: 8 }}
                    >
...
const filterEmptyTables = (tables: TableType[]): TableType[] => tables.filter((t) => !isTableEmpty(t));
...
export const PriceEditModal: React.FC<PriceEditModalProps> = ({
    ...
    const isGroupsDisabled = !hasPermission(PRICE_PAGE_SCOPES_ACTIONS.PRICE_UPDATE_GROUPS);
    const isNameDisabled = !hasPermission(PRICE_PAGE_SCOPES_ACTIONS.PRICE_UPDATE_NAME);
    const isDescriptionDisabled = !hasPermission(PRICE_PAGE_SCOPES_ACTIONS.PRICE_UPDATE_DESCRIPTION);
    ...
    useEffect(() => {
        ...
        if (mode === "update") {
            ...
            setGroups(g);
            setTables(t);
            snapshotRef.current = JSON.stringify({ name: n, description: d, groups: g, tables: t });
            ...
            setGroups([]);
            setTables([]);
            snapshotRef.current = JSON.stringify({ name: "", description: "", groups: [], tables: [] });
            setActiveTabKey("general");
        }
    ...
    const handleDuplicateTable = (idx: number) => {
        ...
        const updated = [...tables.slice(0, idx + 1), copy, ...tables.slice(idx + 1)];
        setTables(updated);
    ...
    const handleSubmit = async () => {
        ...
        const payload = { name, description, groups, price_tables: filterEmptyTables(tables) };
        let success = false;
    ...
    const generalDirty = useMemo(() => {
        ...
        return snap.name !== name || snap.description !== description || JSON.stringify(snap.groups) !== JSON.stringify(groups);
    ...
    const modalTitle = (
        <div className="flex items-center gap-2 flex-wrap">
            <span>
                {isEditing ? "Редактировать услугу" : isDuplicating ? "Добавить услугу (дубль)" : "Добавить услугу"}
            </span>
            {isEditing && name && (
    ...
    const tabLabel = (text: string, dirty: boolean, error: boolean) => (
        <span style={{ position: "relative", paddingRight: dirty || error ? 10 : 0 }}>
            {text}
            {(dirty || error) && (
                <span
                    style={{
    ...
    const footerRight: React.ReactNode[] = [
        <Button key="close" color="default" variant="outlined" icon={<CloseOutlined />} onClick={handleClose}>
            Закрыть
        </Button>,
    ];
        ...
        <div style={{ maxWidth: 720 }}>
            ...
            <div className="mb-4 flex flex-col gap-2">
                ...
                {validationErrors.name ? (
                    <div className="text-sm text-red-500">{validationErrors.name.join(", ")}</div>
                ) : (
            ...
            <div className="mb-4 flex flex-col gap-2">
                ...
                {validationErrors.description ? (
                    <div className="text-sm text-red-500">{validationErrors.description.join(", ")}</div>
                ) : (
                ...
                <Alert
                    ...
                    description={
                        <span>
                            ...
                            <Button type="link" size="small" onClick={handleAddTable} style={{ padding: 0 }}>
                                Добавить таблицу
                            </Button>
                        </span>
                    }
                        ...
                        <div style={{ padding: "16px 0", minHeight: 400 }}>
                            ...
                            {activeTableIndex !== null && tables[activeTableIndex] && (
                                <TableTabPane
                                    ...
                                    onTableChange={(updated) => handleUpdateTable(activeTableIndex, updated)}
                                    onDuplicateTable={() => handleDuplicateTable(activeTableIndex)}
