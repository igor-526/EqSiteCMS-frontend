  ...
  const columns = [
    {
      ...
      key: "groups",
      dataIndex: "groups",
      render: (groups: PriceGroupSimpleOutDto[]) => {
        return groups.map((group) => (
          <span className="m-1" key={group.id}>
      ...
      filterIcon: (
        <SearchOutlined
          style={{
            color:
              (Array.isArray(filters.groups) && filters.groups.length > 0) ||
              (typeof filters.groups === "string" && filters.groups.length > 0)
                ? "#1677ff"
                : undefined,
          }}
        />
        ...
        <>
          <div style={{ padding: 8, minWidth: 250, maxWidth: 350 }}>
            <ListFilter
              ...
              filterKey="groups"
              filterData={priceGroupsOptions.map((option) => ({
                key: option.key,
                label: option.label,
                value: option.label,
              }))}
    ...
    },
    {
      ...
      dataIndex: "slug",
      render: (slug: string) => (
        <span className="text-blue-900 text-sm">{trimText(slug, 40)}</span>
      ),
    },
    {
      title: "Действия",
      key: "actions",
      render: (record: PriceOutDto) => (
