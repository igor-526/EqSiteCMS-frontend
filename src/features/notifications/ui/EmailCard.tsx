...
export function EmailCard(props: Props) {
  return (
    <Card title="Email для уведомлений">
      ...
      {!props.loading && props.error ? <Alert type="error" showIcon message={props.error} /> : null}
      {!props.loading && !props.error && !props.email ? (
        <Space direction="vertical" size="middle">
          <Typography.Text type="secondary">
            Email не привязан. Добавьте адрес, чтобы получать доступные уведомления.
          </Typography.Text>
          <Button type="primary" onClick={props.onCreate}>Добавить email</Button>
        </Space>
      ) : null}
      {!props.loading && !props.error && props.email ? (
        <Space direction="vertical" size="middle" className="w-full">
          ...
          <Space wrap>
            ...
            <Button danger onClick={props.onDelete}>Удалить</Button>
            {!props.email.approved ? (
              <Button loading={props.pending} disabled={props.pending} onClick={props.onResend}>
