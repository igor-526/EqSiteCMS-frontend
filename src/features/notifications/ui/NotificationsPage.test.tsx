...
function handlers(
  emailResponse: Response = HttpResponse.json(
    { detail: "Email not found" },
    { status: 404 },
  ),
  settingsResponse: Response = HttpResponse.json([setting]),
) {
  server.use(
...
describe("NotificationsPage", () => {
  ...
  it("renders history placeholder and settings missing-email empty state", async () => {
    ...
    expect(
      screen.getByText("История уведомлений появится позже."),
    ).toBeInTheDocument();
    await openSettings();
  ...
  it("shows loading and load errors", async () => {
    server.use(
      http.get(api("/emails/me"), async () => {
        await delay(2000);
        return HttpResponse.json({ detail: "email failed" }, { status: 500 });
      }),
      http.get(api("/notification-settings"), async () => {
        await delay(2000);
    ...
    expect(
      await screen.findByText("email failed", {}, { timeout: 4000 }),
    ).toBeInTheDocument();
  ...
  it("validates create, preserves backend error, prevents double submit and refreshes on success", async () => {
    ...
    server.use(
      ...
      http.post(api("/emails"), async () => {
        ...
        await delay(30);
        return HttpResponse.json(
          { id: "e1", user_id: "u1", email: "new@example.ru", approved: false },
          { status: 201 },
        );
      }),
    );
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: "Добавить email" }),
    );
  ...
  it.each([false, true])(
    "shows conditional delete warning and keeps modal on error approved=%s",
    async (approved) => {
      handlers(
        HttpResponse.json({
          id: "e1",
      ...
      server.use(
        http.delete(api("/emails/u1"), () =>
          HttpResponse.json({ detail: "Delete denied" }, { status: 403 }),
        ),
      );
      renderWithCmsProviders(<NotificationsPage />);
      await openSettings();
      ...
      expect(
        screen.getByText(
          approved
            ? /Подтверждённый email будет удалён/
  ...
  it("resends confirmation with success feedback", async () => {
    ...
    server.use(
      http.post(api("/emails/send-confirmation"), () =>
    ...
    await userEvent.click(
      await screen.findByRole("button", {
        name: "Отправить подтверждение повторно",
      }),
    );
  ...
  it("shows ineligible empty state", async () => {
    ...
    await openSettings();
    expect(
      await screen.findByText("Для вашей роли нет доступных событий"),
    ).toBeInTheDocument();
  });
  ...
  it("commits checkbox only after success and preserves it after failure", async () => {
    ...
    let calls = 0;
    server.use(
      http.patch(api("/notification-settings/callback/email"), async () => {
        calls += 1;
        await delay(30);
        return calls === 1
          ? HttpResponse.json({ detail: "Forbidden" }, { status: 403 })
          : HttpResponse.json({ ...setting, enabled: true });
      }),
    );
    renderWithCmsProviders(<NotificationsPage />);
    await openSettings();
    ...
    expect(checkbox).not.toBeChecked();
    expect(await screen.findByText("Forbidden")).toBeInTheDocument();
    const retryCheckbox = await screen.findByRole("switch", {
      name: "Обратный звонок: Email",
    });
