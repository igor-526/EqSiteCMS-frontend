...
import type { EmailOutDto, NotificationSettingOutDto } from "@/types/api/notifications";
...
const isMissingEmail = (detail: string) =>
...
export function useNotifications() {
  ...
  const [settingPendingKey, setSettingPendingKey] = useState<string | null>(null);
  ...
  const loadEmail = useCallback(async () => {
    ...
    } else if (isMissingEmail(result.data.detail)) {
      setEmail(null);
    } else {
  ...
  const loadSettings = useCallback(async () => {
    ...
    else setSettingsError(result.data.detail || "Не удалось загрузить настройки");
  ...
  useEffect(() => {
    void loadEmail();
    void loadSettings();
  }, [loadEmail, loadSettings]);

  const openEmailModal = useCallback((mode: Exclude<EmailModalMode, null>) => {
  ...
  const submitEmail = useCallback(async () => {
    ...
    if (!validated.success) {
      setEmailValidationError(validated.error.issues[0]?.message ?? "Некорректный email");
      return false;
    }
  ...
  const resendConfirmation = useCallback(async () => {
    ...
    setEmailPending(false);
    if (isApiSuccess(result)) {
      notification.success({ title: "Письмо отправлено", description: "Проверьте почту." });
      return true;
    }
    notification.error({ title: "Не удалось отправить письмо", description: result.data.detail });
    return false;
  }, [notification, user]);

  const toggleSetting = useCallback(async (setting: NotificationSettingOutDto) => {
    ...
    if (isApiSuccess(result) && result.data) {
      setSettings((current) => current.map((item) =>
        item.event_code === result.data!.event_code && item.channel_code === result.data!.channel_code
          ? result.data!
          : item,
      ));
      return true;
    }
    const detail = isApiError(result) ? result.data.detail : "Не удалось изменить настройку";
    setSettingsError(detail);
    notification.error({ title: "Настройка не сохранена", description: detail });
    return false;
  }, [notification]);

  return {
    email, emailLoading, emailLoadError, settings, settingsLoading, settingsError,
    emailModalMode, emailDraft, emailValidationError, deleteOpen, mutationError,
    emailPending, settingPendingKey, setEmailDraft, setDeleteOpen, openEmailModal,
    closeEmailModal, submitEmail, deleteCurrentEmail, resendConfirmation,
    toggleSetting, reloadEmail: loadEmail, reloadSettings: loadSettings,
  };
}
