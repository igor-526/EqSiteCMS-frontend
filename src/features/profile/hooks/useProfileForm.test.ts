...
describe("useProfileForm — API boundary", () => {
    ...
    it("updateProfile returns updated data from MSW", async () => {
        ...
        const result = await updateProfile({ first_name: "Алексей", last_name: "Иванов", middle_name: null });
        expect(result.status).toBe("ok");
        if (result.status === "ok") {
    ...
    it("getMyProfile returns error on 401", async () => {
        server.use(
            http.get(apiUrl("/users/me"), () => HttpResponse.json({ detail: "Not authenticated" }, { status: 401 })),
        );
        const result = await getMyProfile();
...
describe("useProfileForm — hook unit", () => {
    ...
    it("reset returns form to loaded values and clears isDirty", async () => {
        ...
        act(() => { result.current.setField("first_name", "Измененное"); });
    ...
    it("save calls updateProfile and shows success toast, clears isDirty", async () => {
        server.use(
            http.get(apiUrl("/users/me"), () => HttpResponse.json(mockProfile)),
            http.patch(apiUrl("/users/me"), () => HttpResponse.json({ ...mockProfile, first_name: "Новый" })),
        );

        const { result } = renderHook(() => useProfileForm());
        await waitFor(() => expect(result.current.loading).toBe(false));

        act(() => { result.current.setField("first_name", "Новый"); });
        expect(result.current.isDirty).toBe(true);

        await act(async () => { await result.current.save(); });

        expect(notificationMock.success).toHaveBeenCalledWith(expect.objectContaining({ title: "Успешно" }));
        expect(result.current.isDirty).toBe(false);
    ...
    it("save shows error toast on backend error, does not clear isDirty", async () => {
        ...
        act(() => { result.current.setField("first_name", "Другое"); });
