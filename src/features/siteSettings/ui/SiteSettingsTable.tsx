...
const getSettingTypeTag = (type: SiteSettingType) => {
  switch (type) {
    ...
    default:
      return (
        <Tag color="gray" variant="outlined">
          Неизвестный тип
        </Tag>
      );
  }
  ...
  const columns = [
    {
      ...
      render: (dataKey: string) => (
        <span className="text-gray-600 font-bold">{trimText(dataKey, 40)}</span>
      ),
      filterIcon: (
        <SearchOutlined
          style={{ color: filters.key ? "#1677ff" : undefined }}
        />
      ),
      filterDropdown: (
        <>
