...
import {
  fetchCreateSiteSetting,
  fetchDeleteSiteSetting,
  fetchSiteSettingList,
  fetchUpdateSiteSetting,
} from "../services/siteSettingsService";
import {
  siteSettingCreateSchema,
  siteSettingUpdateSchema,
} from "../validators/siteSettings";
...
export const useSiteSettings = () => {
  ...
  const [siteSettingsFilters, setSiteSettingsFilters] =
    useState<SiteSettingListQueryParams>(defaultSiteSettingsFilters);
  const [siteSettingsTotal, setSiteSettingsTotal] = useState<number>(0);
  const [siteSettingsLoading, setSiteSettingsLoading] =
    useState<boolean>(false);
  const [siteSettingsValidationErrors, setSiteSettingsValidationErrors] =
  ...
  const createSiteSetting = useCallback(
    async (createData: SiteSettingsCreateInDto) => {
  ...
  const updateSiteSetting = useCallback(
    async (siteSettingId: UUID, updateData: SiteSettingsUpdateInDto) => {
  ...
  const deleteSiteSetting = useCallback(
    async (siteSettingId: UUID) => {
