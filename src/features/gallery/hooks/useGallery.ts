...
export const useGallery = () => {
  ...
  const [pricesFilterOptions, setPricesFilterOptions] = useState<
    SelectProps["options"]
  >([]);
  const [pricesFilterSearchValue, setPricesFilterSearchValue] =
    useState<string>("");
  ...
  const [horsesFilterOptions, setHorsesFilterOptions] = useState<
    SelectProps["options"]
  >([]);
  const [horsesFilterSearchValue, setHorsesFilterSearchValue] =
    useState<string>("");
  ...
  const loadPhotos = useCallback(
    async (append: boolean = false, filters?: PhotoListQueryParams) => {
  ...
  const loadPrices = useCallback(async () => {
    ...
    const response = await fetchPriceList({
      ...
      offset: 0,
      name:
        pricesFilterSearchValue.length === 0 ? null : pricesFilterSearchValue,
      sort: ["name"],
    });
    switch (response.status) {
      case API_STATUS.OK:
        setPricesFilterOptions(
      ...
      case API_STATUS.ERROR:
        toast.error({
          title: "Ошибка",
          description:
            response.data?.detail || "Неизвестная ошибка при загрузке цен",
        });
        break;
      default:
        toast.error({
  ...
  const setNewFilters = useCallback(
    (filters: Partial<PhotoListQueryParams>) => {
      setPhotosFilters((prev) => {
  ...
  const handlePricesFilterChange = useCallback(
    (values: UUID[]) => {
  ...
  const handleHorsesFilterChange = useCallback(
    (values: UUID[]) => {
  ...
  const deletePhotoBatch = useCallback(
    async (photos: PhotoOutDto[]): Promise<boolean> => {
