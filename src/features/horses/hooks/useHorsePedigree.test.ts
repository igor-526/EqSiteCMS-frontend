...
describe("useHorsePedigree", () => {
    ...
    it("resets candidate offset on search and calls candidate service with limit/offset only", async () => {
        ...
        await waitFor(() => expect(serviceMocks.fetchAvailablePedigree).toHaveBeenCalledTimes(1));
    ...
    it("surfaces 401 mutation denial and keeps picker open", async () => {
        ...
        expect(serviceMocks.fetchSetHorsePedigree).toHaveBeenCalledWith(selectedHorse.id, {
            sire_id: candidateId,
        });
        expect(result.current.operationError).toBe("Authentication failed");
        expect(result.current.pickerIntent).toEqual({ mode: "sire", action: "add" });
    });
    ...
    it("surfaces 403 mutation denial without sending page based candidate params", async () => {
        ...
        await waitFor(() => expect(serviceMocks.fetchAvailablePedigree).toHaveBeenCalled());
        ...
        expect(result.current.pickerIntent).toEqual({ mode: "dam", action: "replace" });
    });
    ...
    it("successful mutation refreshes opened modal pedigree and invalidates table", async () => {
        ...
        const { result } = renderHook(() => useHorsePedigree(initialHorse, true, onChanged));
        act(() => {
            result.current.openPicker({ mode: "sire", action: "add" });
        });
        await waitFor(() => expect(result.current.candidatesTotal).toBe(1));
        ...
        expect(serviceMocks.fetchHorse).toHaveBeenCalledWith("atlas", { pedigree: 1 });
        expect(result.current.pickerIntent).toBeNull();
        expect(result.current.horse?.pedigree.sire).toMatchObject({ name: "New Sire" });
    });
