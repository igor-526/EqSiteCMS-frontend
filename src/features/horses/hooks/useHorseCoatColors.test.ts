...
describe("horse coat short-name API boundary", () => {
  beforeEach(() =>
    server.use(
      http.get(apiUrl("/horses/coat_colors"), () =>
  ...
  it("serializes short-name query with limit/offset and returns success/empty", async () => {
    let url = "";
    server.use(
      http.get(apiUrl("/horses/coat_colors"), ({ request }) => {
        url = request.url;
        return HttpResponse.json({ items: [coat], total: 1 });
      }),
    );
    const response = await fetchHorseCoatColorList({
      short_name: "гн",
      sort: ["-short_name"],
      limit: 10,
      offset: 20,
  ...
  it("serializes create/update bodies including empty value", async () => {
    ...
    server.use(
      http.post(apiUrl("/horses/coat_colors"), async ({ request }) => {
        bodies.push(await request.json());
        return HttpResponse.json(coat);
      }),
      http.patch(apiUrl(`/horses/coat_colors/${id}`), async ({ request }) => {
        bodies.push(await request.json());
        return HttpResponse.json(coat);
      }),
    );
  ...
  it.each([401, 403, 422, 500])(
    "surfaces HTTP %s as API error",
    async (status) => {
      server.use(
        http.post(apiUrl("/horses/coat_colors"), () =>
          HttpResponse.json({ detail: "denied" }, { status }),
        ),
      );
      await expect(
        fetchCreateHorseCoatColor({ name: "Гнедая", short_name: "Гн." }),
  ...
  it("validates 63 characters and rejects 64", () => {
    expect(
      horseCoatColorCreateSchema.safeParse({
        name: "Гнедая",
        short_name: "г".repeat(63),
      }).success,
    ).toBe(true);
    expect(
      horseCoatColorCreateSchema.safeParse({
        name: "Гнедая",
        short_name: "г".repeat(64),
      }).success,
    ).toBe(false);
  });
  ...
  it("resets offset for search/sort/page-size but preserves page changes", async () => {
    ...
    await waitFor(() =>
      expect(result.current.horseCoatColorsLoading).toBe(false),
    );
    act(() =>
      result.current.setHorseCoatColorsFilters((prev) => ({
        ...prev,
        offset: 50,
      })),
    );
    ...
    expect(result.current.horseCoatColorsFilters).toMatchObject({
      limit: 50,
      offset: 0,
    });
  });
  ...
  it("retains validation state and surfaces backend denial", async () => {
    ...
    await act(async () => {
      expect(await result.current.createHorseCoatColor({ name: "" })).toBe(
        false,
      );
    });
    ...
    await act(async () => {
      expect(
        await result.current.createHorseCoatColor({ name: "Гнедая" }),
      ).toBe(false);
    });
