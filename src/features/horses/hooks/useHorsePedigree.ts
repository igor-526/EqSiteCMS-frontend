...
export const useHorsePedigree = (
  ...
  const canUpdatePedigree = hasPermission(
    HORSES_PAGE_SCOPES_ACTIONS.UPDATE_HORSE_PEDIGREE,
  );

  const [horse, setHorse] = useState<HorseWithPedigreeOutDto | null>(
    initialHorse,
  );
  const [pickerIntent, setPickerIntent] = useState<PedigreePickerIntent | null>(
    null,
  );
  ...
  const [selectedCandidateId, setSelectedCandidateId] = useState<UUID | null>(
    null,
  );
  ...
  const loadCandidates = useCallback(async () => {
    ...
    if (isApiSuccess(response)) {
      ...
      if (
        selectedCandidateId &&
        !items.some((item) => item.id === selectedCandidateId)
      ) {
  ...
  const refreshHorsePedigree = useCallback(async (): Promise<boolean> => {
    ...
    const detailResponse = await fetchHorse(horse.slug || horse.id.toString(), {
      pedigree: 1,
    });
    if (
      isApiSuccess(detailResponse) &&
      detailResponse.data &&
      "pedigree" in detailResponse.data
    ) {
    ...
    setOperationError(
      isApiError(detailResponse)
        ? detailResponse.data.detail
        : PEDIGREE_REFRESH_ERROR,
    );
    return false;
  }, [horse?.id, horse?.slug]);
  ...
  const buildCandidatePayload =
    useCallback((): HorseSetPedigreeInDto | null => {
