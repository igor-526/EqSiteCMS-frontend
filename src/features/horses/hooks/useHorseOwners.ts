...
import {
  fetchCreateHorseOwner,
  fetchDeleteHorseOwner,
  fetchHorseOwnerList,
  fetchUpdateHorseOwner,
} from "../services/horseOwnerService";
import {
  horseOwnerCreateSchema,
  horseOwnerUpdateSchema,
} from "../validators/horseOwners";
...
export const useHorseOwners = () => {
  ...
  const [horseOwnersFilters, setHorseOwnersFilters] =
    useState<HorseOwnerListQueryParams>(defaultHorseOwnersFilters);
  ...
  const [horseOwnersValidationErrors, setHorseOwnersValidationErrors] =
  ...
  const createHorseOwner = useCallback(
    async (createData: HorseOwnerCreateInDto) => {
  ...
  const updateHorseOwner = useCallback(
    async (horseOwnerId: UUID, updateData: HorseOwnerUpdateInDto) => {
  ...
  const deleteHorseOwner = useCallback(
    async (horseOwnerId: UUID) => {
