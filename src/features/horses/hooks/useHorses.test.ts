...
describe("src/api/horses — API boundary", () => {
  ...
  it.each([
    ...
    "horseUpdate surfaces %s with preserved backend detail",
    async (status, detail) => {
      server.use(
        http.patch(apiUrl(`/horses/${horseId}`), () =>
  ...
  it("horseList serializes services as repeated keys and omits an empty list", async () => {
    ...
    expect(new URL(requests[0]).searchParams.getAll("services")).toEqual([
      serviceIdOne,
      serviceIdTwo,
    ]);
...
describe("useHorses detail navigation support", () => {
  ...
  it("getHorseDetail performs GET detail when related horse is outside current table list", async () => {
    ...
    let detail: Awaited<ReturnType<typeof result.current.getHorseDetail>> =
      null;
    await act(async () => {
      detail = await result.current.getHorseDetail("outside-filter", {
        pedigree: 1,
      });
    });
  ...
  it("service filter apply and clear preserve repeated values and reset offset", () => {
    ...
    act(() =>
      result.current.setHorsesFilters({
        services: [serviceIdOne, serviceIdTwo],
      }),
    );
    expect(result.current.horsesFilters.services).toEqual([
      serviceIdOne,
      serviceIdTwo,
    ]);
    expect(result.current.horsesFilters.offset).toBe(0);
