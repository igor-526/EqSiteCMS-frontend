...
describe("useHorsePageActionScopes", () => {
  ...
  it("reports anonymous users as missing CREATE_HORSE scope", () => {
    ...
    expect(
      result.current.hasPermission(HORSES_PAGE_SCOPES_ACTIONS.CREATE_HORSE),
    ).toBe(false);
  });
  ...
  it("allows UPDATE_HORSE_PEDIGREE with the same admin scopes as UPDATE_HORSE", () => {
    ...
    const { result } = renderHook(() => useHorsePageActionScopes());
    expect(
      result.current.hasPermission(HORSES_PAGE_SCOPES_ACTIONS.UPDATE_HORSE),
    ).toBe(true);
    expect(
  ...
  it("blocks DELETE_HORSE when user has no scope", () => {
    ...
    const { result } = renderHook(() => useHorsePageActionScopes());
    expect(
      result.current.hasPermission(HORSES_PAGE_SCOPES_ACTIONS.DELETE_HORSE),
    ).toBe(false);
  });
  ...
  it("renders create button as enabled when scope is present", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Добавить лошадь" }),
    ).toBeEnabled();
  });
