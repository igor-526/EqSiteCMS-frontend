...
import { findHorseInListById, toHorseWithPedigreeOrNull } from "../lib/horseSelection";
...
type UseHorsesPageHorseActionsParams = {
    ...
    setSelectedHorse: React.Dispatch<React.SetStateAction<HorseOutDto | HorseWithPedigreeOutDto | null>>;
...
export const useHorsesPageHorseActions = ({
    ...
    const resolveHorseFromPedigreeRelation = useCallback(
        async (horse: HorseOutDto, requirePedigree = false) => {
            ...
            return await getHorseDetail(horse.slug || horse.id.toString(), { pedigree: 1 as const });
        },
        [getHorseDetail, horses],
    );

    const handleOpenHorseModal = useCallback((horseId: UUID | null) => {
        if (horseId) {
    ...
    const handlePhotosClick = useCallback((horseId: UUID) => {
        const horse = findHorseInListById(horses, horseId);
        if (horse) {
            setSelectedHorse(horse as HorseOutDto);
    ...
    const handlePedigreeClick = useCallback((horseId: UUID) => {
        const horse = findHorseInListById(horses, horseId);
        if (!horse) {
            toast.error({ title: "Лошадь не найдена. Попробуйте обновить страницу." });
            return;
    ...
    const handleEditFromPedigree = useCallback(async (horse: HorseOutDto) => {
    ...
    const handleOpenPedigreeFromPedigree = useCallback(async (horse: HorseOutDto) => {
