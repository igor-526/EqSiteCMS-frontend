...
import {
  fetchCreateHorseCoatColor,
  fetchDeleteHorseCoatColor,
  fetchHorseCoatColorList,
  fetchUpdateHorseCoatColor,
} from "../services/horseCoatColorService";
import {
  horseCoatColorCreateSchema,
  horseCoatColorUpdateSchema,
} from "../validators/horseCoatColors";
...
export const useHorseCoatColors = () => {
  ...
  const [horseCoatColors, setHorseCoatColors] = useState<
    HorseCoatColorOutDto[]
  >([]);
  const [horseCoatColorsFilters, setHorseCoatColorsFiltersState] =
    useState<HorseCoatColorListQueryParams>(defaultHorseCoatColorsFilters);
  const [horseCoatColorsTotal, setHorseCoatColorsTotal] = useState<number>(0);
  const [horseCoatColorsLoading, setHorseCoatColorsLoading] =
    useState<boolean>(false);
  const [horseCoatColorsValidationErrors, setHorseCoatColorsValidationErrors] =
  ...
  const setHorseCoatColorsFilters = useCallback(
    (value: SetStateAction<HorseCoatColorListQueryParams>) => {
  ...
  const createHorseCoatColor = useCallback(
    async (createData: HorseCoatColorCreateInDto) => {
      const validatedData = horseCoatColorCreateSchema.safeParse(createData);
      if (!validatedData.success) {
        setHorseCoatColorsValidationErrors(
          zodErrorNormalize(validatedData.error),
        );
        return false;
      }
  ...
  const updateHorseCoatColor = useCallback(
    async (horseCoatColorId: UUID, updateData: HorseCoatColorUpdateInDto) => {
      ...
      const response = await fetchUpdateHorseCoatColor(
        horseCoatColorId,
        updateData,
      );
      switch (response.status) {
  ...
  const deleteHorseCoatColor = useCallback(
    async (horseCoatColorId: UUID) => {
