...
import {
  HorseServiceListQueryParams,
  HorseServiceOutDto,
  HorseServiceCreateInDto,
  HorseServiceUpdateInDto,
} from "@/types/api/horseServices";
import {
  fetchCreateHorseService,
  fetchDeleteHorseService,
  fetchHorseServiceList,
  fetchUpdateHorseService,
} from "../services/horseServicesService";
import {
  horseServiceCreateSchema,
  horseServiceUpdateSchema,
} from "../validators/horseServices";
...
export const useHorseServices = () => {
  ...
  const [horseServicesFilters, setHorseServicesFilters] =
    useState<HorseServiceListQueryParams>(defaultHorseServicesFilters);
  const [horseServicesTotal, setHorseServicesTotal] = useState<number>(0);
  const [horseServicesLoading, setHorseServicesLoading] =
    useState<boolean>(false);
  const [horseServicesValidationErrors, setHorseServicesValidationErrors] =
  ...
  const createHorseService = useCallback(
    async (createData: HorseServiceCreateInDto) => {
  ...
  const updateHorseService = useCallback(
    async (horseServiceId: UUID, updateData: HorseServiceUpdateInDto) => {
      ...
      const response = await fetchUpdateHorseService(
        horseServiceId,
        validatedData.data,
      );
      switch (response.status) {
  ...
  const deleteHorseService = useCallback(
    async (horseServiceId: UUID) => {
