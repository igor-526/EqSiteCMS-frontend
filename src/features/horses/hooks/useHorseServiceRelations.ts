...
export const useHorseServiceRelations = (
  onRefreshHorse?: (horseId: UUID) => void,
) => {
  ...
  const [selectedRelation, setSelectedRelation] =
    useState<HorseServiceRelationOutDto | null>(null);
  ...
  const loadAvailableServices = useCallback(
    async (horseId: UUID, search?: string) => {
  ...
  const openUpdateModal = useCallback(
    (relation: HorseServiceRelationOutDto) => {
  ...
  const createRelation = useCallback(
    async (createData: HorseServiceRelationCreateInDto) => {
      if (!selectedHorseId || submittingRef.current) return false;

      const validatedData =
        horseServiceRelationCreateSchema.safeParse(createData);
      ...
      setSubmitting(true);
      try {
        const response = await fetchCreateHorseServiceRelation(
          selectedHorseId,
          createData,
        );
        switch (response.status) {
          ...
          case API_STATUS.ERROR:
            toast.error({
              title: "Ошибка",
              description:
                response?.data?.detail || "Не удалось привязать услугу",
            });
            return false;
          default:
            toast.error({
  ...
  const updateRelation = useCallback(
    async (relationId: UUID, updateData: HorseServiceRelationUpdateInDto) => {
      if (!selectedHorseId || submittingRef.current) return false;

      const validatedData =
        horseServiceRelationUpdateSchema.safeParse(updateData);
      if (!validatedData.success) {
        setValidationErrors(zodErrorNormalize(validatedData.error));
        return false;
      }
      ...
      setSubmitting(true);
      try {
        const response = await fetchUpdateHorseServiceRelation(
          selectedHorseId,
          relationId,
          updateData,
        );
        switch (response.status) {
          ...
          case API_STATUS.ERROR:
            toast.error({
              title: "Ошибка",
              description:
                response?.data?.detail || "Не удалось обновить связь",
            });
            return false;
          default:
            toast.error({
  ...
  const deleteRelation = useCallback(
    async (relationId: UUID) => {
      if (!selectedHorseId || submittingRef.current) return false;

      submittingRef.current = true;
      setSubmitting(true);
      try {
        const response = await fetchDeleteHorseServiceRelation(
          selectedHorseId,
          relationId,
        );
        switch (response.status) {
          ...
          case API_STATUS.ERROR:
            toast.error({
              title: "Ошибка",
              description:
                response?.data?.detail || "Не удалось отвязать услугу",
            });
            return false;
          default:
            toast.error({
