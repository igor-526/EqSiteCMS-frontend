...
export const HorseOwnersCreateUpdateModal: React.FC<
  HorseOwnersCreateUpdateModalProps
> = ({
  open,
  onClose,
  selectedHorseOwner,
  ...
  const [type, setType] = useState<HorseOwnerTypeEnum>(
    HorseOwnerTypeEnum.PERSON,
  );
  ...
  useEffect(() => {
    if (open) {
      ...
      if (selectedHorseOwner) {
        ...
        setPhoneNumbers(
          phoneNumbersStringify(selectedHorseOwner.phone_numbers || []),
        );
      } else {
        setName("");
        setDescription("");
  ...
  if (selectedHorseOwner) {
    ...
    footer.push(
      <Button
        ...
        type="primary"
        onClick={() =>
          onUpdate(selectedHorseOwner.id, {
            name: name,
            description: description,
            type: type,
            address: address,
            phone_numbers: phoneNumbersParse(phoneNumbers),
          })
        }
      >
        <EditOutlined />
    ...
    >
      <div className="mb-6 flex flex-col gap-2">
        ...
        <Radio.Group
          ...
          onChange={(e) => {
            onResetValidation();
            setType(e.target.value as HorseOwnerTypeEnum);
          }}
          optionType="button"
        />
      </div>
      <div className="mb-6 flex flex-col gap-2">
        <label htmlFor="createHorseOwnerNameInput">
      ...
      <div className="mb-6 flex flex-col gap-2">
        <label htmlFor="createHorseOwnerDescriptionInput">
          Описание владельца
        </label>
        <Input
      ...
      <div className="mb-6 flex flex-col gap-2">
        <label htmlFor="createHorseOwnerPhoneNumbersInput">
          Телефоны владельца
        </label>
        <TextArea
        ...
        <div className="text-sm text-gray-500">
          Введите номера телефонов по одному на строку в формате +79991234567
        </div>
        {validationErrors.hasOwnProperty("phone_numbers") && (
          <div className="text-sm text-red-500">
            {validationErrors.phone_numbers.join("\n")}
          </div>
        )}
      </div>
    </Modal>
