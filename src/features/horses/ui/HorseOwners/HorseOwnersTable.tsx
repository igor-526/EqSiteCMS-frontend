  ...
  const columns = [
        ...
        <>
          <div style={{ padding: 8, minWidth: 250, maxWidth: 350 }}>
            <ListFilter
              ...
              filterKey="type"
              filterData={horseOwnerTypesOptions.map((option) => ({
                key: option.key,
                label: option.label,
                value: option.value,
              }))}
        ...
        <>
          <div style={{ padding: 8 }}>
            <StringFilter
              ...
              onChange={(value) =>
                setFilters({ ...filters, description: value ?? null })
              }
    ...
    },
    {
      ...
      render: (address: string | null) => (
        <span>{trimText(address ?? "", 40)}</span>
      ),
      filterIcon: (
        <SearchOutlined
          style={{ color: filters.address ? "#1677ff" : undefined }}
        />
      ),
      filterDropdown: (
        <>
          <div style={{ padding: 8 }}>
            <StringFilter
              ...
              onChange={(value) =>
                setFilters({ ...filters, address: value ?? null })
              }
    ...
    },
    {
      ...
      render: (phone_numbers: string[]) => (
        <span>{phone_numbers.join("\n")}</span>
      ),
      filterIcon: (
        <SearchOutlined
          style={{ color: filters.phone_numbers ? "#1677ff" : undefined }}
        />
      ),
      filterDropdown: (
        <>
          <div style={{ padding: 8 }}>
            <StringFilter
              ...
              onChange={(value) =>
                setFilters({ ...filters, phone_numbers: value ?? null })
              }
