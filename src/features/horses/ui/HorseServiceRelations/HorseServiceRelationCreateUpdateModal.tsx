...
export type HorseServiceRelationCreateUpdateModalProps = {
  ...
  onUpdate: (
    relationId: UUID,
    updateData: HorseServiceRelationUpdateInDto,
  ) => void;
...
export const HorseServiceRelationCreateUpdateModal: React.FC<
  HorseServiceRelationCreateUpdateModalProps
> = ({
  open,
  onClose,
  selectedRelation,
  ...
  const [selectedService, setSelectedService] =
    useState<HorseServiceRelationAvailableServiceDto | null>(null);
  const [descriptionOverride, setDescriptionOverride] = useState<string>("");
  const [priceFormatterOverride, setPriceFormatterOverride] =
    useState<PriceFormatter>(PriceFormatter.equal);
  ...
  useEffect(() => {
    if (open) {
      ...
      if (selectedRelation) {
        ...
        setPriceFormatterOverride(
          selectedRelation.price_formatter || PriceFormatter.equal,
        );
  ...
  const selectedServiceInfo = isEditMode
    ? {
        ...
        price_formatter: selectedRelation.price_formatter,
      }
    : selectedService;
  ...
  return (
    <Modal
      open={open}
      title={
        isEditMode ? "Изменить связь с услугой" : "Добавить услугу к лошади"
      }
      onCancel={onClose}
      footer={footer}
    >
      ...
      <div className="mb-6 flex flex-col gap-2">
        ...
        <TextArea
          placeholder={
            selectedServiceInfo?.description || "Описание для этой лошади"
          }
        ...
        {validationErrors.hasOwnProperty("description_override") ? (
          ...
          <div className="text-sm text-gray-500">
            {descriptionOverride.length}/511
          </div>
        )}
      </div>
