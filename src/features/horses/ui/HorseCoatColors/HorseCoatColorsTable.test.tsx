...
vi.mock("@/ui", () => ({
  MainTable: ({
    ...
    <div data-testid="table" data-loading={loading}>
      ...
      {data.map((row) => (
        <button key={row.key} onClick={onRow(row).onClick}>
          {сolumns.map((column) => (
            <span key={column.key}>
  ...
  it("applies/clears search and asc/desc/clear sort with offset reset", async () => {
    ...
    renderWithCmsProviders(
      <HorseCoatColorsTable
        horseCoatColors={[coat]}
        loading={false}
        filters={{ limit: 25, offset: 50, sort: [] }}
    ...
    expect(setFilters).toHaveBeenCalledWith(
      expect.objectContaining({ short_name: "гн", offset: 0 }),
    );
    await userEvent.click(
      screen.getAllByRole("button", { name: "clear search" })[1],
    );
    expect(setFilters).toHaveBeenCalledWith(
      expect.objectContaining({ short_name: undefined, offset: 0 }),
    );
    for (const name of ["asc", "desc", "clear"])
