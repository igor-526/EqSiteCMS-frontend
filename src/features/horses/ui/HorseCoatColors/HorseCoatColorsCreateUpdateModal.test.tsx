...
describe("HorseCoatColorsCreateUpdateModal", () => {
  it("opens create, closes and submits an empty short name", async () => {
    ...
    expect(onCreate).toHaveBeenCalledWith(
      expect.objectContaining({ name: "Серая", short_name: "" }),
    );
  ...
  it("prefills and submits an update short name", async () => {
    ...
    expect(onUpdate).toHaveBeenCalledWith(
      coat.id,
      expect.objectContaining({ short_name: "Гн." }),
    );
  });
  ...
  it("supports 63 chars and renders a backend field error", async () => {
    ...
    renderModal(null, {
      validationErrors: { short_name: ["Недопустимое значение"] },
    });
  ...
  it("renders only description errors beside the description field", () => {
    renderModal(null, {
      validationErrors: { description: ["Ошибка описания", "Повторите ввод"] },
    });
    expect(screen.getByText(/Ошибка описания/)).toHaveTextContent(
      "Ошибка описания Повторите ввод",
    );
  });
  ...
  it("retains entered values after a failed Protected Write", async () => {
    ...
    await userEvent.type(
      screen.getByLabelText("Наименование масти"),
      "Сохранённая масть",
    );
    await userEvent.type(
      screen.getByLabelText("Описание масти"),
