...
export const getHorsePhotoUrl = (horse: Pick<HorseOutDto, "photos"> | null | undefined) => {
    const photos = horse?.photos ?? [];
    return photos.find((photo) => photo.is_main && photo.url)?.url
...
export const getHorseSexText = (sex?: HorseSex | null) => (sex ? SEX_LABELS[sex] ?? "" : "");
export const getHorseSexMark = (sex?: HorseSex | null) => (sex ? SEX_MARKS[sex] ?? "" : "");
...
export const HorsePedigreeCard: React.FC<HorsePedigreeCardProps> = ({
    ...
    const coatColor = horse?.coat_color?.short_name ?? horse?.coat_color?.name ?? "—";
    ...
    return (
        <div style={{ minWidth: compact ? 220 : 240 }}>
            ...
            <div
                data-testid={horse ? "horse-pedigree-card" : "horse-pedigree-empty-card"}
                style={{
                    position: "relative",
                    display: "grid",
