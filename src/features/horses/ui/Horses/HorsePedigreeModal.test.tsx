...
const makeHorse = (overrides: Partial<HorseOutDto> = {}): HorseOutDto => ({
    ...
    name: "Atlas",
    description: null,
    breed: { id: uuid("000000000101"), name: "Thoroughbred", short_name: "TB", slug: "tb" },
    coat_color: { id: uuid("000000000102"), name: "Гнедая", short_name: "Гн.", slug: "bay" },
    height: null,
    sex: "male",
    bdate: "2018-05-20",
    ddate: null,
...
const makePedigreeHorse = (overrides: Partial<HorseWithPedigreeOutDto> = {}): HorseWithPedigreeOutDto => ({
    ...makeHorse(),
    pedigree: {
        sire: makeHorse({
...
describe("HorsePedigreeModal", () => {
    ...
    it("renders missing parent slots and empty foals state", () => {
        ...
        expect(screen.getByText("У этой лошади пока нет зарегистрированных потомков.")).toBeInTheDocument();
        expect(screen.getByRole("button", { name: /\+ Добавить потомка/i })).toBeInTheDocument();
    });
    ...
    it("hides remove and go actions for missing parent slot", async () => {
        ...
        await userEvent.click(screen.getAllByRole("button", { name: "Действия" })[0]);
    ...
    it("disables mutation actions when scope is missing", async () => {
        ...
        expect(screen.getByText(/доступна только для просмотра/i)).toBeInTheDocument();
        expect(screen.getByRole("button", { name: /\+ Добавить потомка/i })).toBeDisabled();
        await userEvent.click(screen.getAllByRole("button", { name: "Действия" })[0]);
        await waitFor(() =>
            expect(screen.getByText("Удалить связь").closest("li")).toHaveClass("ant-dropdown-menu-item-disabled"),
        );
        expect(screen.getByText("Заменить связь").closest("li")).toHaveClass("ant-dropdown-menu-item-disabled");
    });
