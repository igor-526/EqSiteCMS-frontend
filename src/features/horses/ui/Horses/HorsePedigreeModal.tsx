    ...
    async ({ key }) => {
      if (key === "replace") {
        if (kind === "sire")
          openPicker({
            mode: "sire",
            action: relatedHorse ? "replace" : "add",
          });
        if (kind === "dam")
          openPicker({ mode: "dam", action: relatedHorse ? "replace" : "add" });
        if (kind === "foal")
          openPicker({
            mode: "children",
            action: "replace",
            targetFoalId: relatedHorse?.id,
          });
        return;
      }
    ...
    <>
      <Modal
        ...
        title={
          <span>
            Родословная
            {horse ? ` — ${horse.name} (${getHorseSexMark(horse.sex)})` : ""}
          </span>
        }
        onCancel={onClose}
        width={1100}
            ...
            <section>
              ...
              >
                <HorsePedigreeCard
                  ...
                  menuItems={
                    sire
                      ? buildFilledItems(canUpdatePedigree)
                      : buildMissingItems(canUpdatePedigree)
                  }
            ...
            <section>
              ...
              >
                ...
                <Button
                  type="primary"
                  onClick={() =>
                    openPicker({ mode: "children", action: "add" })
                  }
                  ...
                  <Empty
                    description={
                      <span>
                        ...
                        <br />У этой лошади пока нет зарегистрированных
                        потомков.
                      </span>
                    }
                  />
