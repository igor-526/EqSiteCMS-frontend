...
describe("HorsePedigreeCard", () => {
    ...
    it("falls back from formatted date to date mode", () => {
        expect(getHorseBirthDateText(horse({ bdate_mode: "y", bdate: "2020-03-15" }))).toBe("2020");
        expect(getHorseBirthDateText(horse({ bdate_mode: "ym", bdate: "2020-03-15" }))).toBe("03.2020");
        expect(getHorseBirthDateText(horse({ bdate_mode: "hide", bdate: "2020-03-15" }))).toBe("");
    });
    ...
    it("renders geld marker as male sign plus m", () => {
        renderWithCmsProviders(<HorsePedigreeCard horse={horse({ sex: "geld", name: "Merin" })} />);
    ...
    it("renders empty relation slot", () => {
        renderWithCmsProviders(<HorsePedigreeCard horse={null} emptyText="Не указан" />);
