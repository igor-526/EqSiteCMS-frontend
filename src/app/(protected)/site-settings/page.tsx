...
export default function SiteSettingsPage() {
  ...
  const [selectedSiteSetting, setSelectedSiteSetting] =
    useState<SiteSettingOutDto | null>(null);
  const [isSiteSettingModalOpen, setIsSiteSettingModalOpen] =
  ...
  const handleOpenSiteSettingModal = (siteSettingId: UUID | null) => {
    if (siteSettingId) {
      const siteSetting = siteSettings.find(
        (siteSetting) => siteSetting.id === siteSettingId,
      );
      if (siteSetting) {
        ...
        toast.error({
          title:
            "Настройка сайта не найдена. Попробуйте обновить страницу и повторить попытку.",
        });
        return;
      }
    } else {
  ...
  const handleCreateSiteSetting = async (
    siteSetting: SiteSettingsCreateInDto,
  ) => {
  ...
  const handleUpdateSiteSetting = async (
    siteSettingId: UUID,
    siteSetting: SiteSettingsUpdateInDto,
  ) => {
