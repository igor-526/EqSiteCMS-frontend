...
import { TABLE_LAYOUT } from "@/lib/constants";
...
export const MainTable: React.FC<MainTableProps> = ({
  ...
  const [tableHeight, setTableHeight] = useState<number>(TABLE_LAYOUT.defaultHeight);
  ...
  useEffect(() => {
    const updateTableHeight = () => {
      if (containerRef.current && filtersRef.current) {
        ...
        const newHeight = containerRect.height - filtersRect.height - TABLE_LAYOUT.headerOffset;
        setTableHeight(Math.max(TABLE_LAYOUT.minHeight, newHeight));
      }
    };
  ...
  const tableKey = useMemo(() => {
    return currentSort && currentSort.length > 0
      ? currentSort.join(",")
      : "no-sort";
  }, [currentSort]);
  ...
  return (
    <div className="h-full overflow-y-hidden" ref={containerRef}>
      {filtersElements && (
        <div ref={filtersRef} className="flex mb-3 justify-between flex-wrap">
          {filtersElements}
        </div>
      )}
      <Table
