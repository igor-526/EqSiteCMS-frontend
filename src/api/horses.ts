...
export const horseList = (
  ...
  ApiResult<ApiListPaginatedResponseType<HorseOutDto | HorseWithPedigreeOutDto>>
> => {
  const paramtrizedUrl = addQueryParamsToUrl("/horses", params);
  return apiFetch<
    ApiListPaginatedResponseType<HorseOutDto | HorseWithPedigreeOutDto>
  >(paramtrizedUrl, options);
};
...
export const horseGet = (
  ...
  return apiFetch<HorseOutDto | HorseWithPedigreeOutDto>(
    paramtrizedUrl,
    options,
  );
};
...
export const horseDelete = (id: UUID): Promise<ApiResult<null>> => {
